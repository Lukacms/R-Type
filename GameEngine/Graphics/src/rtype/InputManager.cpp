...
void InputManager::update(const sf::Window& window) {
    for (int key = sf::Keyboard::Key::A; key != sf::Keyboard::Key::KeyCount; ++key) {
        keyMap[static_cast<sf::Keyboard::Key>(key)] = sf::Keyboard::isKeyPressed(static_cast<sf::Keyboard::Key>(key));
    }

    for (int button = sf::Mouse::Button::Left; button != sf::Mouse::Button::ButtonCount; ++button) {
        mouseButtonMap[static_cast<sf::Mouse::Button>(button)] = sf::Mouse::isButtonPressed(static_cast<sf::Mouse::Button>(button));
    }
}
