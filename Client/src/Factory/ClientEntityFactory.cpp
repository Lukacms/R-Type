...
{
    if (type == "Enemy") {
        return create_enemy(entity, ecs_manager);
    }
    if (type == "Player") {
        return create_player(entity, ecs_manager);
    }
    if (type == "OtherPlayer") {
        return create_other_player(entity, ecs_manager);
    }
    if (type == "Bullet") {
        return create_bullet(entity, ecs_manager);
    }
                                                   ...
                                                   rtype::ECSManager &ecs_manager)
{
    ...
    rtype::SpriteComponent tmp;
    tmp.texture.loadFromFile("../../assets/Spaceship.png");
    tmp.sprite.setTexture(tmp.texture);
    sf::IntRect rect(0, 0, 33, 15);
    tmp.sprite.setScale(2, 2);
    tmp.sprite.setTextureRect(rect);
    sprite.insert_at(entity, rtype::SpriteComponent{tmp});
    return entity;
}
